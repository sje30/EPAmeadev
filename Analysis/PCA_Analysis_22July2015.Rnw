...
# data files
data.file<-paste(data.dir,"data_all_wellLevel_July13.RData", sep="") # data.all
data.merged.file<-paste(data.dir,"date_all_merged_July13.RData", sep="") #called total

load( data.file )
load (data.merged.file ) #total is name of df

# rename for consistency
names(total)[which(names(total)=="plate")]="Plate.SN"
names(total)[which(names(total)=="div")]="DIV"
df = total
df$DIV<-as.numeric(substring(df$DIV,4,5))
df$ddp<-paste( df$date, df$DIV, df$Plate.SN, sep="_" )
df$dwp<-paste( df$date, df$well, df$Plate.SN, sep="_" )

# add data labels
names(df)[names(df)=='0.05']="tiling0.05"

vars.want<-c(  "file.name",
                "date" ,               
                "Plate.SN",
                "DIV", 
                "well",
...
vars.labels<-c( 
                ...
                DIV="DIV", 
                well="well",
                fir.rate="firing rate (hz)", 
                bursty="% electrodes >= 1 burst per minute",
                bursts.per.min="burst rate (min)", 
                ns.n="# network spikes", 
                cv.IBIs="coef. variation of IBI",  
                mean.freq="within burst firing rate", 
                mean.dur="burst durtion (s)",
                tiling0.05="tiling coefficient", 
                durn.m="network spike duration (s)", 
                per.spikes.in.burst="% spikes in burst",
                peak.m="# electrodes at peak of network spikes",
                cv.ISIs="coef. of variation of ISI in burst",
                ddp ="date-div-plate"  ,
                dwp ="date-well-plate" )


label(df) = lapply(names( vars.labels ), 
                     function(x) label(df[,x]) = vars.labels[x])




# first a little summary about number of plates
num.plates<-length(unique(paste( df$date, df$Plate.SN, sep="_" )))
print(paste("There are ", num.plates," plates total." ,sep=""))

print("Number of plates by DIV")
a<-table(paste( df$date,  df$Plate.SN, sep="_" ) , df$DIV)
colSums(a)



colors.div<-c("#fec44f", "#fe9929","#d95f0e","#993404")
...
vars.want.quant<-setdiff(vars.want, redundant.vars )
  ...
  ###########
  # data prep
  index.want<-which(is.element( names(df),c("ddp", vars.want.quant)) )
  df.sub1<-df[ ,index.want ]
  plate.df.all<-aggregate(df.sub1, by=list(df.sub1$ddp), median, na.rm=T)
  plate.df.all<-plate.df.all[,-length(plate.df.all[1,]) ]# remove ddp
...
col.ind.plate<-unlist(lapply(strsplit(plate.df.all[,1],spli="_"), function(x) x[2]) )
